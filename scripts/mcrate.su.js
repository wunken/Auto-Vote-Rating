async function vote(first) {
    if (document.querySelector('div[class="error"]')) {
        ...
        if (request.message.includes('аккаунт заблокирован') || request.message.includes('ВК ID заблокирован') || request.message.includes('Неправильный токен ВК') || request.message.includes('не прошли reCapatcha') || request.message.includes('Получены не все данные, либо данные неверны')) {
